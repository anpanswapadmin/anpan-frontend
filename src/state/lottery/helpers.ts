...
export const fetchLottery = async (lotteryId: string): Promise<LotteryResponse> => {
  try {
    ...
    const serializedAnpanPerBracket = anpanPerBracket.map((anpanInBracket) => ethersToSerializedBigNumber(anpanInBracket))
    ...
    return {
      ...
      priceTicketInAnpan: ethersToSerializedBigNumber(priceTicketInAnpan),
      discountDivisor: discountDivisor?.toString(),
      treasuryFee: treasuryFee?.toString(),
      firstTicketId: firstTicketId?.toString(),
      lastTicketId: lastTicketId?.toString(),
      amountCollectedInAnpan: ethersToSerializedBigNumber(amountCollectedInAnpan),
      finalNumber,
      anpanPerBracket: serializedAnpanPerBracket,
...
export const useProcessLotteryResponse = (
  ...
  const priceTicketInAnpan = useMemo(() => {
    return new BigNumber(priceTicketInAnpanAsString)
  }, [priceTicketInAnpanAsString])

  const amountCollectedInAnpan = useMemo(() => {
    return new BigNumber(amountCollectedInAnpanAsString)
  }, [amountCollectedInAnpanAsString])
