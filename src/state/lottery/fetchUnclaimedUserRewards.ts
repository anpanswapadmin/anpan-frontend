...
const fetchAnpanRewardsForTickets = async (
  winningTickets: LotteryTicket[],
): Promise<{ ticketsWithUnclaimedRewards: LotteryTicket[]; anpanTotal: BigNumber }> => {
  ...
  const anpanRewards = await multicallv2(lotteryV2Abi, calls)

  const anpanTotal = anpanRewards.reduce((accum: BigNumber, anpanReward: ethers.BigNumber[]) => {
    return accum.plus(new BigNumber(anpanReward[0].toString()))
  }, BIG_ZERO)

  const ticketsWithUnclaimedRewards = winningTickets.map((winningTicket, index) => {
    return { ...winningTicket, anpanReward: anpanRewards[index] }
  })
  return { ticketsWithUnclaimedRewards, anpanTotal }
}
...
export const getWinningTickets = async (
  ...
  if (unclaimedWinningTickets.length > 0) {
    const { ticketsWithUnclaimedRewards, anpanTotal } = await fetchAnpanRewardsForTickets(unclaimedWinningTickets)
    return { ticketsWithUnclaimedRewards, allWinningTickets, anpanTotal, roundId }
  }

  if (allWinningTickets.length > 0) {
    return { ticketsWithUnclaimedRewards: null, allWinningTickets, anpanTotal: null, roundId }
  }
