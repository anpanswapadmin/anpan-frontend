...
import { Flex, Text, Skeleton, Link, Button, ArrowForwardIcon } from '@anpanswap/uikit'
...
const LotteryCardContent = () => {
  ...
  const anpanPriceBusdAsString = usePriceAnpanBusd().toString()

  const anpanPrizesText = t('%anpanPrizeInUsd% in ANPAN prizes this round', { anpanPrizeInUsd: anpanPriceBusdAsString })
  const [pretext, prizesThisRound] = anpanPrizesText.split(anpanPriceBusdAsString)

  const anpanPriceBusd = useMemo(() => {
    return new BigNumber(anpanPriceBusdAsString)
  }, [anpanPriceBusdAsString])
  ...
  useEffect(() => {
    ...
    const fetchCurrentLotteryPrize = async () => {
      const { amountCollectedInAnpan } = await fetchLottery(lotteryId)
      const prizeInBusd = anpanPriceBusd.times(amountCollectedInAnpan)
  ...
  }, [lotteryId, slowRefresh, setCurrentLotteryPrize, anpanPriceBusd])
    ...
    <>
      <Flex flexDirection="column" mt="48px">
        ...
        <Text color="white" mb="40px">
          {t('Buy tickets with ANPAN, win ANPAN if your numbers match')}
        </Text>
      </Flex>
