...
import { usePriceAnpanBusd, useLottery } from 'state/hooks'
import { fetchUserTicketsAndLotteries } from 'state/lottery'
import useTheme from 'hooks/useTheme'
import useTokenBalance, { FetchStatus } from 'hooks/useTokenBalance'
import useApproveConfirmTransaction from 'hooks/useApproveConfirmTransaction'
import { useAnpan, useLotteryV2Contract } from 'hooks/useContract'
...
const BuyTicketsModal: React.FC<BuyTicketsModalProps> = ({ onDismiss }) => {
  ...
  const [userNotEnoughAnpan, setUserNotEnoughAnpan] = useState(false)
  const lotteryContract = useLotteryV2Contract()
  const anpanContract = useAnpan()
  const { toastSuccess } = useToast()
  const { balance: userAnpan, fetchStatus } = useTokenBalance(getAnpanAddress())
  ...
  const stringifiedUserAnpan = userAnpan.toJSON()
  const memoisedUserAnpan = useMemo(() => new BigNumber(stringifiedUserAnpan), [stringifiedUserAnpan])

  const anpanPriceBusd = usePriceAnpanBusd()
  const dispatch = useAppDispatch()
  const hasFetchedBalance = fetchStatus === FetchStatus.SUCCESS
  const userAnpanDisplayBalance = getFullDisplayBalance(userAnpan, 18, 3)
  ...
  const getTicketCostAfterDiscount = useCallback(
    (numberTickets: BigNumber) => {
      const totalAfterDiscount = priceTicketInAnpan
        .times(numberTickets)
        .times(discountDivisor.plus(1).minus(numberTickets))
        .div(discountDivisor)
      return totalAfterDiscount
    },
    [discountDivisor, priceTicketInAnpan],
  )

  const getMaxTicketBuyWithDiscount = useCallback(
    (numberTickets: BigNumber) => {
      const costAfterDiscount = getTicketCostAfterDiscount(numberTickets)
      const costBeforeDiscount = priceTicketInAnpan.times(numberTickets)
      const discountAmount = costBeforeDiscount.minus(costAfterDiscount)
      const ticketsBoughtWithDiscount = discountAmount.div(priceTicketInAnpan)
      const overallTicketBuy = numberTickets.plus(ticketsBoughtWithDiscount)
      return { overallTicketBuy, ticketsBoughtWithDiscount }
    },
    [getTicketCostAfterDiscount, priceTicketInAnpan],
  )

  const validateInput = useCallback(
    (inputNumber: BigNumber) => {
      const limitedNumberTickets = limitNumberByMaxTicketsPerBuy(inputNumber)
      const anpanCostAfterDiscount = getTicketCostAfterDiscount(limitedNumberTickets)

      if (anpanCostAfterDiscount.gt(userAnpan)) {
        setUserNotEnoughAnpan(true)
      } else if (limitedNumberTickets.eq(maxNumberTicketsPerBuyOrClaim)) {
        setMaxTicketPurchaseExceeded(true)
      } else {
        setUserNotEnoughAnpan(false)
        setMaxTicketPurchaseExceeded(false)
      }
    },
    [limitNumberByMaxTicketsPerBuy, getTicketCostAfterDiscount, maxNumberTicketsPerBuyOrClaim, userAnpan],
  )

  useEffect(() => {
    const getMaxPossiblePurchase = () => {
      const maxBalancePurchase = memoisedUserAnpan.div(priceTicketInAnpan)
      const limitedMaxPurchase = limitNumberByMaxTicketsPerBuy(maxBalancePurchase)
      let maxPurchase

      // If the users' max ANPAN balance purchase is less than the contract limit - factor the discount logic into the max number of tickets they can purchase
  ...
  useEffect(() => {
    ...
    const costBeforeDiscount = priceTicketInAnpan.times(numberOfTicketsToBuy)
  ...
  }, [ticketsToBuy, priceTicketInAnpan, discountDivisor, getTicketCostAfterDiscount])
  ...
  const { isApproving, isApproved, isConfirmed, isConfirming, handleApprove, handleConfirm } =
    useApproveConfirmTransaction({
      onRequiresApproval: async () => {
        try {
          const response = await anpanContract.allowance(account, lotteryContract.address)
      ...
      onApprove: () => {
        return anpanContract.approve(lotteryContract.address, ethers.constants.MaxUint256)
      },
  ...
  const getErrorMessage = () => {
    if (userNotEnoughAnpan) return t('Insufficient ANPAN balance')
  ...
  return (
    <StyledModal title={t('Buy Tickets')} onDismiss={onDismiss} headerBackground={theme.colors.gradients.cardHeader}>
      ...
      <BalanceInput
        isWarning={userNotEnoughAnpan || maxTicketPurchaseExceeded}
        placeholder="0"
        value={ticketsToBuy}
        onUserInput={handleInputChange}
        currencyValue={
          anpanPriceBusd.gt(0) &&
          `~${ticketsToBuy ? getFullDisplayBalance(priceTicketInAnpan.times(new BigNumber(ticketsToBuy))) : '0.00'} ANPAN`
        }
      />
      <Flex alignItems="center" justifyContent="flex-end" mt="4px" mb="12px">
        <Flex justifyContent="flex-end" flexDirection="column">
          {(userNotEnoughAnpan || maxTicketPurchaseExceeded) && (
      ...
      <Flex flexDirection="column">
        <Flex mb="8px" justifyContent="space-between">
          ...
          <Text color="textSubtle" fontSize="14px">
            {priceTicketInAnpan && getFullDisplayBalance(priceTicketInAnpan.times(ticketsToBuy || 0))} ANPAN
          </Text>
        </Flex>
        <Flex mb="8px" justifyContent="space-between">
          <Flex>
