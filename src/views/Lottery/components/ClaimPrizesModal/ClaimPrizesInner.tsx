...
import { Flex, Button, Text, AutoRenewIcon, PresentWonIcon } from '@anpanswap/uikit'
import { useTranslation } from 'contexts/Localization'
import { LotteryTicket, LotteryTicketClaimData } from 'config/constants/types'
import { getBalanceAmount } from 'utils/formatBalance'
import { callWithEstimateGas } from 'utils/calls'
import { useLottery, usePriceAnpanBusd } from 'state/hooks'
...
const ClaimInnerContainer: React.FC<ClaimInnerProps> = ({ onSuccess, roundsToClaim }) => {
  ...
  const anpanReward = activeClaimData.anpanTotal
  const dollarReward = anpanReward.times(anpanPriceBusd)
  const rewardAsBalance = getBalanceAmount(anpanReward).toNumber()
  ...
  const handleClaim = async () => {
    ...
    try {
      ...
      if (receipt.status) {
        toastSuccess(
          t('Prizes Collected!'),
          t('Your ANPAN prizes for round %lotteryId% have been sent to your wallet', { lotteryId }),
        )
        setPendingTx(false)
