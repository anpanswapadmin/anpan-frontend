...
import { Modal, Text, Flex, Button, HelpIcon, AutoRenewIcon, useTooltip } from '@anpanswap/uikit'
...
const BountyModal: React.FC<BountyModalProps> = ({ onDismiss, TooltipComponent }) => {
  ...
  const totalYieldToDisplay = getBalanceNumber(totalPendingAnpanHarvest, 18)
  ...
  const handleConfirmClick = async () => {
    const tx = await callWithEstimateGas(anpanVaultContract, 'harvest', [], 5000)
    setPendingTx(true)
    const receipt = await tx.wait()
    if (receipt.status) {
      toastSuccess(t('Bounty collected!'), t('ANPAN bounty has been sent to your wallet.'))
      setPendingTx(false)
      onDismiss()
    } else {
    ...
    <Modal title={t('Claim Bounty')} onDismiss={onDismiss} headerBackground={theme.colors.gradients.cardHeader}>
      ...
      <Flex alignItems="flex-start" justifyContent="space-between">
        ...
        <Flex flexDirection="column">
          <Balance bold value={anpanBountyToDisplay} decimals={7} unit=" ANPAN" />
      ...
      <Flex alignItems="center" justifyContent="space-between">
        ...
        <Balance color="textSubtle" value={totalYieldToDisplay} unit=" ANPAN" />
      </Flex>
