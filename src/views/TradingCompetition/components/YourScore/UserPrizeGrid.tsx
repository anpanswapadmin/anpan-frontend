...
const UserPrizeGrid: React.FC<{ userTradingInformation?: UserTradingInformationProps }> = ({
  ...
  const { anpanReward, dollarValueOfAnpanReward } = useCompetitionAnpanRewards(userAnpanRewards)
        ...
        <tr>
          <BoldTd>
            <Flex flexDirection="column">
              <Text bold>{anpanReward.toFixed(2)}</Text>
              {dollarValueOfAnpanReward ? (
                <Text fontSize="12px" color="textSubtle">
                  ~{dollarValueOfAnpanReward} USD
                </Text>
              ) : (
