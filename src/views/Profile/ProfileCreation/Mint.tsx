...
import { Card, CardBody, Heading, Text } from '@anpanswap/uikit'
import { useWeb3React } from '@web3-react/core'
import { useTranslation } from 'contexts/Localization'
import useApproveConfirmTransaction from 'hooks/useApproveConfirmTransaction'
import { DEFAULT_TOKEN_DECIMAL } from 'config'
import { useAnpan, useBunnyFactory } from 'hooks/useContract'
...
const minimumAnpanBalanceToMint = new BigNumber(MINT_COST).multipliedBy(DEFAULT_TOKEN_DECIMAL)

const Mint: React.FC = () => {
  const [variationId, setVariationId] = useState<Nft['variationId']>(null)
  const { actions, minimumAnpanRequired, allowance } = useProfileCreation()

  const { account } = useWeb3React()
  const anpanContract = useAnpan()
  const bunnyFactoryContract = useBunnyFactory()
  const { t } = useTranslation()
  const hasMinimumAnpanRequired = useHasAnpanBalance(minimumAnpanBalanceToMint)
  const { isApproving, isApproved, isConfirmed, isConfirming, handleApprove, handleConfirm } =
    useApproveConfirmTransaction({
      onRequiresApproval: async () => {
        // TODO: Move this to a helper, this check will be probably be used many times
        try {
          const response = await anpanContract.allowance(account, bunnyFactoryContract.address)
      ...
      onApprove: () => {
        return anpanContract.approve(bunnyFactoryContract.address, allowance.toJSON())
      },
      onConfirm: () => {
  ...
  return (
    <>
      ...
      <Text as="p" mb="24px">
        {t('You can change your profile pic later if you get another approved Anpans Collectible.')}
      </Text>
      <Card mb="24px">
        <CardBody>
          ...
          <Text as="p" mb="24px" color="textSubtle">
            {t('Cost: %num% ANPAN', { num: MINT_COST })}
          </Text>
          {nfts.map((nft) => {
            ...
            return (
              <SelectionCard
                ...
                disabled={isApproving || isConfirming || isConfirmed || !hasMinimumAnpanRequired}
              >
          ...
          {!hasMinimumAnpanRequired && (
            <Text color="failure" mb="16px">
              {t('A minimum of %num% ANPAN is required', { num: MINT_COST })}
            </Text>
          )}
          <ApproveConfirmButtons
            isApproveDisabled={variationId === null || isConfirmed || isConfirming || isApproved}
            isApproving={isApproving}
            isConfirmDisabled={!isApproved || isConfirmed || !hasMinimumAnpanRequired}
