...
const minimumAnpanToRegister = new BigNumber(REGISTER_COST).multipliedBy(DEFAULT_TOKEN_DECIMAL)
...
const UserName: React.FC = () => {
  ...
  const { teamId, selectedNft, userName, actions, minimumAnpanRequired, allowance } = useProfileCreation()
  ...
  const hasMinimumAnpanRequired = useHasAnpanBalance(minimumAnpanToRegister)
  const [onPresentConfirmProfileCreation] = useModal(
    <ConfirmProfileCreationModal
      ...
      account={account}
      teamId={teamId}
      minimumAnpanRequired={minimumAnpanRequired}
      allowance={allowance}
    />,
    false,
  )
    ...
    <>
      ...
      {!hasMinimumAnpanRequired && (
        <Text color="failure" mt="16px">
          {t('A minimum of %num% ANPAN is required', { num: REGISTER_COST })}
        </Text>
      )}
    </>
  )
