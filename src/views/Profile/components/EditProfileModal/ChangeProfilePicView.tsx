...
import { Button, InjectedModalProps, Skeleton, Text } from '@anpanswap/uikit'
  ...
  const { isApproving, isApproved, isConfirmed, isConfirming, handleApprove, handleConfirm } =
    useApproveConfirmTransaction({
      onApprove: () => {
        ...
        return contract.approve(getAnpansProfileAddress(), selectedNft.tokenId)
      },
      onConfirm: () => {
        if (!profile.isActive) {
    ...
    <>
      ...
      {!isLoading && nftsInWallet.length === 0 && (
        <>
          ...
          <Text as="p" color="textSubtle" mb="24px">
            {t('Make sure you have a Anpans Collectible in your wallet and try again!')}
          </Text>
        </>
      )}
