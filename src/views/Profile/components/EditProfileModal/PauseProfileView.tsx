...
import { AutoRenewIcon, Button, Checkbox, Flex, InjectedModalProps, Text } from '@anpanswap/uikit'
...
const PauseProfilePage: React.FC<PauseProfilePageProps> = ({ onDismiss }) => {
  ...
  const { numberAnpanToReactivate } = useGetProfileCosts()
  const { t } = useTranslation()
  const anpansProfileContract = useProfileContract()
  ...
  const handleDeactivateProfile = async () => {
    const tx = await anpansProfileContract.pauseProfile()
  ...
  return (
    <>
      ...
      <Text as="p" color="textSubtle" mb="24px">
        {t('Cost to reactivate in the future: %cost% ANPAN', { cost: getBalanceNumber(numberAnpanToReactivate) })}
      </Text>
