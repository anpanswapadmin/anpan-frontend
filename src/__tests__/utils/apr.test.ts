...
describe('getFarmApr', () => {
  it(`returns null when parameters are missing`, () => {
    const { anpanRewardsApr, lpRewardsApr } = getFarmApr(null, null, null, null)
    expect(anpanRewardsApr).toBeNull()
    expect(lpRewardsApr).toEqual(0)
  })
  it(`returns null when APR is infinite`, () => {
    const { anpanRewardsApr, lpRewardsApr } = getFarmApr(BIG_ZERO, BIG_ZERO, BIG_ZERO, '')
    expect(anpanRewardsApr).toBeNull()
    expect(lpRewardsApr).toEqual(0)
  })
  it(`get the correct pool APR`, () => {
    const { anpanRewardsApr, lpRewardsApr } = getFarmApr(BIG_TEN, new BigNumber(1), new BigNumber(100000), '')
    expect(anpanRewardsApr).toEqual(4204800)
    expect(lpRewardsApr).toEqual(0)
  })
